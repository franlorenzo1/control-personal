...
- **Monto inicial**: representa tu base (por ejemplo, sueldo) y podés editarlo en cualquier momento desde el mismo formulario.
- **Ingresos extra**: permite agregar plata adicional con detalle (changa, transferencia, etc.).
- **Gasto fijo**: ahora podés elegir si es:
  - **en cuotas** (monto total + cantidad de cuotas + mes del primer débito), o
  - **mensual** (monto mensual, sin pedir mes de primer débito).
- El sistema muestra además:
  - cuotas restantes y mes de finalización para gastos en cuotas,
  - y en gastos mensuales los considera vigentes de forma continua,
  - con resumen mensual de gastos vigentes.
